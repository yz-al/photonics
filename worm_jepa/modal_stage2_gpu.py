...
_PASS = {
    ...
    "WORM_S3_MECHEQ": "1" if os.environ.get("WORM_S3_MODE") == "mecheq" else os.environ.get("WORM_S3_MECHEQ", "0"),
...
def main():
    ...
    if os.environ.get("WORM_S3_MODE") == "curve":              # clean learning curve: 1 container/seed, aggregate
        ...
        return
    if os.environ.get("WORM_S3_MODE") == "mecheq":            # distill boundary decision -> mechanistic equation
        print("[modal] mechanistic equation: distill boundary decision to image primitives ...", flush=True)
        print("[modal] cache:", prepare.remote(), flush=True)
        result = run_seed.remote(0)
        art = os.path.join(HERE, "artifacts"); os.makedirs(art, exist_ok=True)
        for fn in ("mecheq.json", "segment3d.json"):
            with open(os.path.join(art, fn), "w") as f:
                json.dump(result, f, indent=2)
        print(json.dumps(result.get("mechanistic_equation", result), indent=2))
        return
